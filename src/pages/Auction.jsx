...
function Auction() {
  ...
  const { items, status } = useAuctionSearch(name);
  const dataToShow = useLoadAuctionItems(items, name);
  ...
  return (
    <Template>
      <section className="container mx-auto md:pt-36 pt-28 flex flex-col items-center justify-center pb-10">
        ...
        <div className="gap-8 w-full flex gap-4 mt-16 flex-wrap justify-center xl:px-20 md:px-0 sm:px-32 px-14 transition">
          {status === "loading" && (
            <div className="mt-10">
              <svg
                aria-hidden="true"
          ...
          {status === "error" && (
            <div className="font-bold text-xl">
              Opps.. Try to reload page. Some errors opeared..
            </div>
          )}
          {status === "loaded" && (
            <>
          ...
          {items.length === 0 && status === "loaded" && (
            <div className="font-bold text-xl">0 result for your search</div>
          )}
        </div>
      </section>
    </Template>
